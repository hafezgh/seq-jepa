from utils import off_diagonal, gather_from_all
import torch
...
class SimCLREquiMod(nn.Module):
    def __init__(self, num_classes, latent_size, learn_action_emb, action_emb_size, equi_factor, temp, cifar_resnet, distributed=False):
        super().__init__()
...
class SEN_Contrastive(nn.Module):
    def __init__(self, num_classes, latent_size, learn_action_emb, action_emb_size, temp, cifar_resnet, distributed=False):
        super().__init__()
        self.cifar_resnet = cifar_resnet
        self.distributed = distributed
        self.encoder = torchvision.models.resnet18(pretrained=False, zero_init_residual=True)
    ...
    def add_probes(self, latent_type):
        ...
        if latent_type == "aug":
            self.blur_regressor = nn.Linear(dim, 1)
            self.crop_regressor = nn.Sequential(
                nn.Linear(dim,1024), nn.ReLU(), nn.Linear(1024,1024), nn.ReLU(), nn.Linear(1024, 4))
            self.jitter_regressor = nn.Sequential(
                nn.Linear(dim,1024), nn.ReLU(), nn.Linear(1024,1024), nn.ReLU(), nn.Linear(1024, 4))
        elif latent_type == "rotcolor":
            self.rot_regressor = nn.Sequential(
...
class SIE(nn.Module):
    def __init__(self, n_classes, inv_coeff, var_coeff, cov_coeff, equi_factor, latent_size, cifar_resnet, distributed=False):
        super().__init__()
        self.cifar_resnet = cifar_resnet
        self.distributed = distributed
        ### ResNet encoder
    ...
    def forward(self, x, y, rel_latent=None):
        ...
        x_inv_proj = self.projector_inv(x_inv)
        y_inv_proj = self.projector_inv(y_inv)
        x_equi_proj = self.projector_equi(x_equi)
        y_equi_proj = self.projector_equi(y_equi)

        # Concatenate both parts to apply the regularization on the whole vectors
        x_cat = torch.cat((x_inv_proj, x_equi_proj), dim=1)
        y_cat = torch.cat((y_inv_proj, y_equi_proj), dim=1)

        #======================================
        #           Inv part
        #======================================
        repr_loss_inv = F.mse_loss(x_inv_proj, y_inv_proj)

        #======================================
        #           Equi part
        #======================================
        y_equi_pred = self.predictor(x_equi_proj.unsqueeze(1), rel_latent)
        repr_loss_equi = F.mse_loss(y_equi_pred, y_equi_proj)
        
        #======================================
        #           Common part (variance/covariance)
        #======================================
        # Gather for variance/covariance in distributed setting
        if self.distributed:
            x_gathered = gather_from_all(x_cat)
            y_gathered = gather_from_all(y_cat)
            y_equi_pred_gathered = gather_from_all(y_equi_pred)
        else:
            x_gathered = x_cat
            y_gathered = y_cat
            y_equi_pred_gathered = y_equi_pred

        batch_size = x_gathered.size(0)
        
        y_equi_pred_centered = y_equi_pred_gathered - y_equi_pred_gathered.mean(dim=0)
        std_y_equi_pred = torch.sqrt(y_equi_pred_centered.var(dim=0) + 0.0001)
        pred_std_loss = torch.mean(F.relu(1 - std_y_equi_pred)) / 2

        x_centered = x_gathered - x_gathered.mean(dim=0)
        y_centered = y_gathered - y_gathered.mean(dim=0)
...
class SimCLR(nn.Module):
    def __init__(self, n_classes, temp, cifar_resnet, distributed=False):
        super().__init__()
        self.cifar_resnet = cifar_resnet
        self.distributed = distributed
        self.encoder = torchvision.models.resnet18(pretrained=False, zero_init_residual=True)
...
class VICReg(nn.Module):
    def __init__(self, n_classes, inv_coeff, var_coeff, cov_coeff, cifar_resnet, distributed=False):
        super().__init__()
        self.cifar_resnet = cifar_resnet
        self.distributed = distributed
        ### ResNet encoder
    ...
    def forward(self, x1, x2, rel_latent=None):
        ...
        # Gather for variance/covariance in distributed setting
        if self.distributed:
            y1_gathered = gather_from_all(y1)
            y2_gathered = gather_from_all(y2)
        else:
            y1_gathered = y1
            y2_gathered = y2
        
        batch_size = y1_gathered.size(0)
        y1_centered = y1_gathered - y1_gathered.mean(dim=0)
        y2_centered = y2_gathered - y2_gathered.mean(dim=0)
        
        std_x = torch.sqrt(y1_centered.var(dim=0) + 0.0001)
        std_y = torch.sqrt(y2_centered.var(dim=0) + 0.0001)
        
        var_loss = torch.mean(F.relu(1 - std_x)) / 2 + torch.mean(F.relu(1 - std_y)) / 2

        cov_x = (y1_centered.T @ y1_centered) / (batch_size - 1)
        cov_y = (y2_centered.T @ y2_centered) / (batch_size - 1)
...
class VICReg_Traj(nn.Module):
    ...
    def forward(self, x, rel_latent=None):
        ...
        # Gather for variance/covariance in distributed setting
        if self.distributed:
            y_l_gathered = gather_from_all(y_l)
            y_c_gathered = gather_from_all(y_c)
        else:
            y_l_gathered = y_l
            y_c_gathered = y_c
        
        global_batch_size = y_l_gathered.size(0)
        y1 = y_l_gathered - y_l_gathered.mean(dim=0)
        y2 = y_c_gathered - y_c_gathered.mean(dim=0)
        ...
        cov_x = (y1.T @ y1) / (global_batch_size - 1)
        cov_y = (y2.T @ y2) / (global_batch_size - 1)
        cov_loss = off_diagonal(cov_x).pow_(2).sum().div(
            self.emb_size
        ) + off_diagonal(cov_y).pow_(2).sum().div(self.emb_size)
